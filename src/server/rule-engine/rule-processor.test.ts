...
describe('rule-processor', () => {
  describe('applyPreTransformRules', () => {
    ...
    describe('REGEX flags', () => {
      ...
      it('should be case-sensitive by default (no fallback flags)', () => {
        ...
        const rules: ImportRule[] = [{
          ...
          conditionColumn: 'description',
          condition: 'REGEX',
          conditionValue: 'uber', // No slashes, should be case-sensitive
          action: 'EXCLUDE',
          actionColumn: '',
          stopProcessing: false,
          rulePhase: 'PRE_TRANSFORM',
        ...
        const processor = new RuleProcessor(rules)
        const result = processor.applyPreTransformRules(table, 'TestBank')
        expect(result.excludedIndices.has(0)).toBe(false) // UBER should NOT match
        expect(result.excludedIndices.has(1)).toBe(true) // uber should match
      })
      ...
      it('should support explicit regex flags /pattern/flags', () => {
        ...
        const rules: ImportRule[] = [{
          ruleName: 'Match Uber Case Insensitive',
          banks: ['All'],
          conditionColumn: 'description',
          condition: 'REGEX',
          conditionValue: '/uber/i',
          action: 'EXCLUDE',
          actionColumn: '',
          stopProcessing: false,
          rulePhase: 'PRE_TRANSFORM',
      ...
      it('should NOT use "i" flag if not in /pattern/flags format', () => {
        const data = [['UBER']]
        const table = new Table(headers, data)
        const rules: ImportRule[] = [{
          ruleName: 'Match Uber',
          ...
          conditionColumn: 'description',
          condition: 'REGEX',
          conditionValue: 'uber', // no slashes, no 'i' flag
          action: 'EXCLUDE',
          actionColumn: '',
          stopProcessing: false,
          rulePhase: 'PRE_TRANSFORM',
        ...
        const processor = new RuleProcessor(rules)
        const result = processor.applyPreTransformRules(table, 'TestBank')
        expect(result.excludedIndices.has(0)).toBe(false) // Should NOT match UBER
      })
    })
  })
